   ...
   <form method="POST" action="/blog/{{ $post->id }}">
    ...
    <div class="space-y-12">
      <div class="border-b border-gray-900/10 pb-12">
        ...
        <p class="mt-1 text-sm/6 text-gray-600">Use this form to update post data.</p>
          ...
          <div class="col-span-full">
            ...
            <div class="mt-2">
              <textarea id="body" name="body" rows="3"
                class=" {{ $errors->has('body') ? 'outline-red-500' : 'outline-gray-300' }} block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6">{{ old('body',$post->body) }}</textarea>
            </div>
            ...
            <div class="flex gap-3">
              <div class="flex h-6 shrink-0 items-center">
                <div class="group grid size-4 grid-cols-1">
                  <input id="published" type="checkbox" name="published" value="1" {{ old('published') ||( !old() && $post->published) ? 'checked' : '' }} aria-describedby="published-description"
              ...
              <div class="text-sm/6">
                ...
                <p id="published-description" class="text-gray-500">Do you want it published or saved as draft.</p>
              </div>
            </div>
          </div>
        </div>
